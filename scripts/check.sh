...
# Self-heal on a fresh machine: fetch cordon once if it isn't here yet, rather
# than erroring with manual setup steps. A no-op when it's already present.
if [ ! -f "$CORDON_HOME/checks/run.mjs" ]; then
  CORDON_HOME="$("$(dirname "$0")/ensure-cordon.sh")"
  export CORDON_HOME
fi
